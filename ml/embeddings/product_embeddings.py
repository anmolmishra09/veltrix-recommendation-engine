...
class ProductEmbedding(BaseEmbedding):
    ...
    def load(self, path: str, format: str = 'csv'):
        ...
        Supported formats:
        - csv: CSV file with columns 'product_id' and 'embedding_0', 'embedding_1', ..., 'embedding_{n-1}'
        - numpy: .npy file containing a matrix and a separate .npy file for IDs (or a pickle)
        - json: JSON file with product_id as key and list of floats as value
        ...
        logger.info(f"Loaded product embeddings for {len(self.id_to_index)} products with dimension {self.embedding_dim}")
    ...
    def _load_from_numpy(self, path: str):
        # Expects two files: path + '.npy' for embeddings and path + '_ids.npy' for IDs
    ...
    def _load_from_json(self, path: str):
        ...
        # Expects a dictionary: {product_id: [embedding values]}
        ids = list(data.keys())
